# USPs per Product & Segment

Unieke verkoopargumenten gerangschikt op impact. Nederlands.

---

## Magic Mirror XL

| # | USP | Waarom het werkt | Beste segment |
|---|---|---|---|
| 1 | Echte DSLR-camera met professionele studiobelichting | Direct herkenbaar verschil met concurrentie | Alle segmenten |
| 2 | Professionele studiobelichting — iedereen ziet er geweldig uit | Verlicht angst "slechte foto's" | Perfectionist, Zakelijk |
| 3 | Direct printen — fysiek aandenken om mee te nemen | Tastbaar resultaat, geen digitaal gedoe | Perfectionist |
| 4 | Tot 5 uur inclusief — ruimste standaard in de markt | Concurrentie rekent per uur (2-3 uur pakketten), langer in overleg mogelijk | Alle segmenten |
| 5 | Eén team voor DJ + photobooth | Minder leveranciers, betere afstemming | Alles-in-één |
| 6 | VIP-setup met rode loper en stijlvolle props | Creëert een "Hollywood-moment" voor gasten | Perfectionist, Zakelijk |
| 7 | AI Photo Experience (+€499) — AI genereert unieke branded foto's | Niemand anders biedt dit, ideaal voor productlanceringen en branding | Zakelijk |
| 8 | Wedding Keychain Station (+€499) — onbeperkt gepersonaliseerde sleutelhangers | Vervangt apart bedankje/aandenken kopen — gasten nemen persoonlijk cadeau mee, bruidsparen besparen op los bedankje | Perfectionist (Bruiloft) |

## Party Booth

| # | USP | Waarom het werkt | Beste segment |
|---|---|---|---|
| 1 | Vanaf €199 — fotoplezier voor elk budget | Laagste instapprijs in premium markt | Slimme Feestganger |
| 2 | Zelf ophalen, 5 minuten opzetten, klaar | Geen monteur nodig = geen extra kosten | Slimme Feestganger |
| 3 | Direct delen op socials | Gasten delen = gratis marketing voor het feest | Slimme Feestganger |
| 4 | Compact — past op elke locatie | Geen ruimte-eisen, werkt overal | Slimme Feestganger |
| 5 | Combineerbaar met Live DJ Set | Upgrade naar compleet entertainment | Alles-in-één |
| 6 | AI Foto's + Filters (+€49) — lelijke achtergrond? wij fixen het | Lost een echt probleem op, laagdrempelige upsell, concurrentie biedt dit niet | Slimme Feestganger |

## DJ Services

| # | USP | Waarom het werkt | Beste segment |
|---|---|---|---|
| 1 | Spotify playlist-optimalisatie — stuur je playlist, krijg een professionele set | Niemand anders biedt dit, vermindert angst "verkeerde muziek" | Alle segmenten |
| 2 | Vanaf €149 inclusief speakers, licht en rookmachine | Markt vraagt €600-1.200 voor professionele DJ, wij zijn 4x goedkoper | Slimme Feestganger, Alles-in-één |
| 3 | Twee DJ-stijlen: Afro/Caribbean (Gianni) of Tech House/House/Deep (Milø) | Keuze die past bij de sfeer van het event | Alle segmenten |
| 4 | Eén team = photobooth + DJ uit dezelfde hand | Geen coördinatie tussen leveranciers | Alles-in-één |
| 5 | Persoonlijke energie — familiebedrijf, geen bureau | Warm, echt, betrokken — voelt niet als een transactie | Perfectionist, Zakelijk |

---

## Cross-Product USPs (PYB als geheel)

| # | USP | Kernboodschap (NL) |
|---|---|---|
| 1 | Echte apparatuur, geen speelgoed | "DSLR camera, professionele studiobelichting — foto's die je wilt inlijsten" |
| 2 | Spotify playlist-optimalisatie | "Stuur je playlist, wij bouwen er een professionele set van" |
| 3 | Familiebedrijf met persoonlijke energie | "Geen bureau, geen verkooppraatjes — wij zijn Richard, Luca en Milo" |
| 4 | Twee instappunten: premium én betaalbaar | "Magic Mirror XL voor de wow-factor, Party Booth voor elk budget" |
| 5 | Eén team voor alles | "Photobooth + DJ uit dezelfde hand — één contactpersoon, één vibe" |

---

## Suggesties ter Verbetering

*Opmerking: deze suggesties veranderen niets aan het huidige aanbod, alleen aan hoe we het presenteren.*

- **"5 uur inclusief"** prominenter maken — ruimste standaard in de markt, concurrentie rekent per uur (2-3 uur pakketten)
- **Spotify-optimalisatie** altijd benoemen bij DJ — het is de sterkste differentiator
- **DSLR-kwaliteit tonen** — voorbeeldfoto's van Magic Mirror XL laten zien (niet vergelijken met tablet, wij bieden zelf ook Party Booth)
- **Familiebedrijf-verhaal** inzetten voor vertrouwen — mensen boeken bij mensen, niet bij bedrijven
- **€149 DJ prijs** beter framen tegen de markt — "Een DJ-bureau rekent €600+, bij ons vanaf €149"
- **'Meest Gekozen' badge verwijderd** van homepage (maart 2026). Anchor via charm pricing €599, social proof, en scarcity.
